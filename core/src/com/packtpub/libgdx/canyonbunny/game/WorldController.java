...
public class WorldController extends InputAdapter
{
    ...
    /**
     * Handles user input for the player character (Bunny Head). User input is only
     * handled if the camera target is the Bunny Head.
     * 
    ...
    /**
     * Defines how the collision between the BunnyHead with a Rock should be
     * handled.
     * 
     ...
     * The collected state of the GoldCoin is set to true and the score value of the
     * GoldCoin is added to the player's score.
     * 
    ...
    /**
     * Defines how the collision between the BunnyHead and a Feather is handled. The
     * collected state of the Feather is set to true and the score value of the
     * Feather is added to the player's score. The feather powerup is applied to the
     * BunnyHead.
     * 
    ...
    private void testCollisions()
    {
        ...
        for (Rock rock : level.rocks)
        {
            ...
            if (!r1.overlaps(r2))
                continue; // if BunnyHead is not colliding with current Rock, continue to next iteration
        ...
        for (GoldCoin goldCoin : level.goldcoins)
        {
            if (goldCoin.collected)
                continue; // ignore GoldCoin that has already been collected
            r2.set(goldCoin.position.x, goldCoin.position.y, goldCoin.bounds.width, goldCoin.bounds.height);
            if (!r1.overlaps(r2))
                continue; // if BunnyHead is not colliding with current GoldCoin, continue to next
                          // iteration
        ...
        for (Feather feather : level.feathers)
        {
            if (feather.collected)
                continue; // ignore Feather that has already been collected
            r2.set(feather.position.x, feather.position.y, feather.bounds.width, feather.bounds.height);
            if (!r1.overlaps(r2))
                continue; // if BunnyHead is not colliding with current Feather, continue to next
                          // iteration
    ...
    /**
     * Creates a variable number of carrots at a given location. Carrots are spawned
     * outside of the camera's view. Carrots have box2d bodies and fixtures to allow
     * collisions.
     * 
     * @param pos the position to spawn the carrots at
     * @param numCarrots the number of carrots to be spawned
     * @param radius how far from pos to spawn carrots in either direction
     */
    private void spawnCarrots(Vector2 pos, int numCarrots, float radius)
    {
        ...
        for(int i = 0; i < numCarrots; i++)
        {
            ...
            // Calculate random spawn position, radius, rotation, and scale
            ...
            float halfHeight = carrot.bounds.width / 2.0f * carrotScale;
